...
| `STEPFUN_CN_API_KEY`            | Stepfun (China) Step Plan auth                   | Using `stepfun-cn` provider                                    | `STEP_API_KEY` is accepted as a fallback. Login checks `https://api.stepfun.com/step_plan/v1/models` |
