...
## Stepfun (China) (`stepfun-cn`)

Stepfun (China) is the Step Plan subscription on the China console (`https://platform.stepfun.com`). It uses the OpenAI Chat Completions transport at `https://api.stepfun.com/step_plan/v1`. The same key on `https://api.stepfun.com/v1` is the separate pay-as-you-go channel and is not this provider. `/login` shows it as Stepfun (China).
...
- **Reasoning effort**: `step-5-preview`, `step-3.7-flash`, `step-3.5-flash`, and `step-router-v1` advertise `low` / `medium` / `high`. `step-3.5-flash-2603` accepts only `low` / `high`. Thinking text arrives as `reasoning` or `reasoning_content`; the completions parser keeps the first non-empty field.
...
### Auth & usage
- **Environment variables**: `STEPFUN_CN_API_KEY`, then `STEP_API_KEY`.
- **API Key Login**: `packages/catalog/src/compat/rules/auth/stepfun-cn.kdl` validates with `GET https://api.stepfun.com/step_plan/v1/models`. The key page is `https://platform.stepfun.com/interface-key`.
- **No Usage Tracking**: No quota module under `packages/ai/src/usage/`.

### Catalog model handling
- **Descriptor**: `stepfun-cn` in `packages/catalog/src/provider-models/descriptors.ts`, default model `step-5-preview`, `dynamicModelsAuthoritative: true`.
...
- **Live roster**: With a key, `stepfunCnModelManagerOptions` fetches `/v1/models` and drops speech, image-generation, and embedding ids via the `stepfun-cn` exclude-models rule. Known ids keep the seeded windows.
